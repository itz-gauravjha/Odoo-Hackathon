...
export default function EmployeeDirectory({ employees, attendance = [], leaves = [], onSelectEmployee, defaultAvatar }) {
  
  const getEmployeeStatus = (emp) => {
    const todayStr = new Date().toDateString();
    
    // Check present (if checked in today)
    const hasCheckedInToday = attendance.some(record => {
      const recordEmpId = record.employeeId?._id || record.employeeId;
      return recordEmpId === emp._id && new Date(record.date).toDateString() === todayStr;
    });
    if (hasCheckedInToday) return 'present';
    
    // Check on leave (if approved leave covers today)
    const hasApprovedLeaveToday = leaves.some(req => {
      const leaveEmpId = req.employeeId?._id || req.employeeId;
      if (leaveEmpId !== emp._id || req.status !== 'Approved') return false;
    ...
    if (hasApprovedLeaveToday) return 'leave';
